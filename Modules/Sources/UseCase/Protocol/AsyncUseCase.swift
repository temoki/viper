...
public final class AnyAsyncUseCase<Input, Output, Failure: Error>: AsyncUseCase {
    ...
    public init<T: AsyncUseCase>(_ base: T) where T.Input == Input, T.Output == Output, T.Failure == Failure {
...
private final class AsyncUseCaseBox<T: AsyncUseCase>: AnyAsyncUseCaseBox<T.Input, T.Output, T.Failure> {
    private let base: T
